...
class ReportController extends Controller
{
  public function indexScore(Request $request)
  {
      ...
      if($search==''){
          ...
          ->select('services.id','services.idcategory','services.idistatus','services.idsupplier',
                   ...
                   ->where('services.idistatus','=',7)->where('services.idcategory','=',1)
        ...
        ->join('suppliers','services.idsupplier','=','suppliers.id')
        ->join('inspdetails','services.id','=','inspdetails.idservice')
        ->select('services.id','services.idcategory','services.idistatus','services.idsupplier','services.action',
  ...
  public function newSfaScore(Request $request)
  {
    ...
    $explodedEv = explode(',',$request->mxEtwo);
    ...
    $score = Inspscore::where('iddetail','=',$idDet)->first();
    $score->mxQone=$request->mxQone;
    $score->mxQtwo=$request->mxQtwo;
    $score->mxEtwo=$fileNameEv;
    $score->mxPthree=$request->mxPthree;
    $score->mxPfour=$request->mxPfour;
    $score->mxQfive=$request->mxQfive;
    $score->mxQSix=$request->mxQSix;
    $score->mxRemarks=$request->mxRemarks;
    $score->mxScore=$request->mxScore;
    $score->db_link=$request->db_link;
    $score->modified_mxby = Auth::user()->user;
    $score->modified_mxby_id = Auth::user()->id;
    $score->upload_report_date = Carbon::today();
  ...
  public function downloadLiveReport(Request $request)
  {
    $id = $request->id;
    //Log::info($id);
    $dl = Monitor::where('monitors.idservice','=',$id)->get();
    $path = $dl[0]->third_evidence;
    $filePath = public_path('files/').$path;
    $headers = array (
      'Content-type : application/doc',
    );

    //return Storage::disk('public')->download($filePath,'name.jpg');
    return  Response::download($filePath,$path,$headers);
